...
import slice_lifecycle_mgr.nsi_manager2repo as nsi_repository
...
def instantiateNSI(nsiId):
    ...
    #instantiates the required NetServices by sending requests to the Sonata SP
    ...
    #checks if all the current instantiations in the Sonata SP are READY to store the info into the NSI object
    ...
    repo_response = nsi_repository.safe_nsi(NSI)
    ...
    if NST.nstUsageState == "NOT_IN_USE":
      ...
      #TODO (when NSI-repo is done): send the NST information to the catalogues --> POST /???
